...
const API_BASE =
  (import.meta?.env?.VITE_API_BASE_URL || "https://paymeback-server-1.onrender.com")
    .replace(/\/$/, "");
...
export default function HomePage() {
  ...
  const wallet = useMemo(() => {
    ...
    for (const p of plans) {
      const remaining =
        coercePositive(p.totalRemaining) ||
        coercePositive(p.remaining) ||
        coercePositive(p.amount) * Math.max(1, toNumber(p.totalPayments) || 1);

      const dir =
        (typeof p.direction === "string" && p.direction.toLowerCase()) ||
        (p.isIncoming ? "incoming" : null) ||
        "incoming"; // default: owed TO you

      if (dir === "outgoing") outgoingRemaining += remaining;
      else incomingRemaining += remaining;
    }
  ...
  return (
    <div className="stack">
      {/* ====== INSIGHT CENTER (locked style) ====== */}
      <section className="card" style={styles.heroCard}>
        <div style={styles.heroGrid}>
          {/* Left: headline, subtext, CTAs */}
          <div style={styles.heroLeft}>
            <div style={styles.heroKicker}>INSIGHT CENTER</div>

            <h1 style={styles.heroTitle}>
              Keep every IOU on<br />schedule without the<br />awkward follow-up
            </h1>

            <p style={styles.heroSub}>
              PayMeBack turns repayments into a calm, transparent workflow.
              Track balances, share gentle reminders, and celebrate when
              everyone settles up.
            </p>

            <div style={styles.heroActions}>
              <a href="/create-iou" className="btn" aria-label="Create IOU">Create IOU →</a>
              <button type="button" className="btn" style={styles.btnGhost}>
                Invite contact
              </button>
            </div>
          </div>

          {/* Right: 2×2 metric tiles with subtle glow */}
          <div style={styles.heroRight}>
            <MetricTile label="OUTSTANDING IOUs" value={plans.length} />
            <MetricTile label="YOU ARE OWED" value={wallet.youAreOwed} money />
            <MetricTile label="YOU OWE" value={wallet.youOwe} money />
            <MetricTile label="NET TOTAL" value={wallet.netTotal} money />
          </div>
        </div>
      </section>
      ...
      <section className="card">
        ...
        {loading ? (
          <div style={{ color: "var(--text-1)", fontSize: 16 }}>Loading…</div>
        ) : (
          <div className="latest-activity">
            ...
            <div
              ...
              aria-hidden
            >
              <div className="who">With</div>
              <div className="note">Note</div>
              <div className="date">Next Payment Date</div>
              <div className="amt">Next Payment</div>
              <div className="total">Total Remaining</div>
            </div>

            {/* graceful empty with retry */}
            {err || plans.length === 0 ? (
              <EmptyState err={err} onRetry={loadPlans} />
            ) : (
              plans.map((p) => (
                <IOURow key={p.id || `${p.with || p.name || "row"}-${Math.random()}`} plan={p} />
              ))
            )}
          </div>
        )}
  ...
  return (
    <div style={styles.tile}>
      ...
      <div style={styles.tileValue}>
        {money ? `$${formatMoney(toNumber(value))}` : String(value)}
      </div>
      <div style={styles.tileDelta} aria-hidden />
    </div>
  );
}
...
function IOURow({ plan }) {
  const who =
    plan.with || plan.counterparty || plan.name || plan.to || plan.from || "Unknown";
  ...
  const totalRemaining =
    coercePositive(plan.totalRemaining) ||
    coercePositive(plan.remaining) ||
    amount * Math.max(1, toNumber(plan.totalPayments) || 1);

  const direction =
    (typeof plan.direction === "string" && plan.direction.toLowerCase()) ||
  ...
  return (
    <div
      className="iou-row"
      style={{ fontSize: 16, border: rowBorder, background: "rgba(255,255,255,0.03)" }}
      title={
        overdue
          ? "Overdue"
          : direction === "outgoing"
    ...
    >
      <div className="who" title={who}>{who}</div>
      <div className="note" title={note}>{note}</div>
      <div className="date">{nextPaymentDate ? fmtDate(nextPaymentDate) : "—"}</div>
      <div className="amt" style={{ color: amtColor }}>${formatMoney(amount)}</div>
      <div className="total" style={{ color: overdue ? RED : GREEN }}>${formatMoney(totalRemaining)}</div>
    </div>
  );
}
    ...
    >
      {err ? (
        <>
          ...
          <button className="btn" onClick={onRetry} type="button" aria-label="Retry loading plans">
            Retry
          </button>
        </>
      ) : (
        <>No activity yet. Click <b>+ Create a new IOU</b> to get started.</>
      )}
    </div>
  );
}
...
const styles = {
  ...
  tile: {
    ...
    padding: 16,
    boxShadow:
      "0 14px 44px rgba(43,107,255,0.15), 0 0 0 1px rgba(255,87,160,0.08) inset, inset 0 1px 0 rgba(255,255,255,0.04)", // blue/pink hybrid glow
  },
